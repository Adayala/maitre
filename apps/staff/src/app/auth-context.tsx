...
// token pasted on the login screen. The owner app is operational/admin-facing,
// so the fixture token is scoped separately from the cashier/waiter apps.
interface AuthState {
...
const FIXTURE_TOKEN_KEY = "maitre.owner.fixtureAccessToken";
  ...
  async function signInWithPassword(signInEmail: string, password: string) {
    ...
    const result = await Promise.race([
      ...
      new Promise<never>((_, reject) =>
        window.setTimeout(() => reject(new Error("El login tardó demasiado. Reintentá.")), SIGN_IN_TIMEOUT_MS),
      ),
    ]);

...
import type { AuditLog, AuditLogQuery, AuditLogPage, AuditLogRepositoryPort } from "../index.js";
...
class FakeAuditLogRepository implements AuditLogRepositoryPort {
  ...
  async query(params: AuditLogQuery): Promise<AuditLogPage> {
    ...
    if (params.resourceType) items = items.filter((i) => i.resourceType === params.resourceType);
    if (params.from) items = items.filter((i) => i.occurredAt >= params.from!);
    if (params.to) items = items.filter((i) => i.occurredAt <= params.to!);
    items = [...items].sort((a, b) => b.occurredAt.getTime() - a.occurredAt.getTime());
...
test("query filters by actorId, resourceType and time range, ordered descending", async () => {
  ...
  await recordAuditLog(
    ...
    { tenantId: "t1", actorType: "USER", actorId: "u1", action: "CREATE", resourceType: "Branch", resourceId: "b1" },
  );
  await recordAuditLog(
    { auditLogs, now: () => new Date("2026-02-01T00:00:00Z") },
    { tenantId: "t1", actorType: "USER", actorId: "u1", action: "UPDATE", resourceType: "Branch", resourceId: "b1" },
  );
  await recordAuditLog(
    { auditLogs, now: () => new Date("2026-01-15T00:00:00Z") },
    { tenantId: "t1", actorType: "USER", actorId: "u2", action: "CREATE", resourceType: "Product", resourceId: "p1" },
  );

...
function Gate() {
  ...
  if (!services.some((service) => service.code === "RESERVATIONS")) {
    return <SubscriptionState title="Maitre Reservations no está contratado para esta sucursal" />;
  }

  return (
    <>
      <SubscriptionCapacity services={services} codes={["RESERVATIONS", "SEATS"]} />
      <HostPage />
    </>
  );
}

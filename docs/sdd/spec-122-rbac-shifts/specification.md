...
Permisos canónicos implementados en workforce:
...
La autorización implementada hoy combina:

- sesión válida;
- selección de tenant vía `X-Tenant-Id`;
- membership activa en ese tenant;
- `roleIds` de la membership resueltos a permisos;
- `branchScopeType` + `branchIds`;
- `externalIdentityId` para los casos de own-access.

`workshift:read_own` y `time:read_own` operan sobre la propia relación laboral inferida por
`Employment.personRef === externalIdentityId`. Lectura sensible y export requieren permisos más
fuertes, branch scope válido y, para export, step-up y auditoría.
...
- `time:export` no implica `workshift:plan`, `workshift:assign` ni `time:adjust_approve`
...
- `labor_policy:manage` autoriza crear, versionar, activar o corregir configuración de policy
  laboral
- `labor_policy:review` no implica `time:read_sensitive`, `time:export` ni `time:adjust_approve`

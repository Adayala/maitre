...
serialTest("Reservation lifecycle: create, confirm, seat", async () => {
  ...
  const visit = await container.visits.findById(tenantId, seat.json().data.visitId);
  assert.ok(visit);
...
serialTest("Reservation list filters by status and no-show transitions a confirmed reservation", async () => {
  const container = await buildContainer();
  const { tenantId, branchId } = await getContext(container);
  const app = await buildApp(container);
  ...
  const firstCreate = await app.inject({
    ...
    url: `/v1/branches/${branchId}/reservations`,
    headers: ownerHeaders(container, tenantId),
    payload: { partySize: 2, startAt: "2026-08-03T20:00:00Z", durationMinutes: 90 },
  });
  ...
  const secondCreate = await app.inject({
    ...
    url: `/v1/branches/${branchId}/reservations`,
    headers: ownerHeaders(container, tenantId),
    payload: { partySize: 3, startAt: "2026-08-03T21:00:00Z", durationMinutes: 60 },
  });
...
serialTest("Reservation create/detail preserve source while list redacts it", async () => {
  const container = await buildContainer();
  const { tenantId, branchId } = await getContext(container);
  const app = await buildApp(container);
...
serialTest("Reservation detail returns the reservation and hides cross-tenant reservations as 404", async () => {
  const container = await buildContainer();
  const { tenantId, branchId } = await getContext(container);
  const app = await buildApp(container);
  const headers = ownerHeaders(container, tenantId);
  ...
  const create = await app.inject({
    ...
    url: `/v1/branches/${branchId}/reservations`,
    headers,
    payload: { partySize: 2, startAt: "2026-08-04T20:00:00Z", durationMinutes: 90 },
  });
  assert.equal(create.statusCode, 201);
  const reservation = create.json().data;
...
serialTest("Reservation list returns empty for unknown or cross-tenant branches", async () => {
  const container = await buildContainer();
  const { tenantId, branchId } = await getContext(container);
  const app = await buildApp(container);
  const headers = ownerHeaders(container, tenantId);
  ...
  const create = await app.inject({
    ...
    url: `/v1/branches/${branchId}/reservations`,
    headers,
    payload: { partySize: 2, startAt: "2026-08-06T20:00:00Z", durationMinutes: 90 },
  });
  assert.equal(create.statusCode, 201);
...
serialTest("Reservation list validates status filter against the domain enum", async () => {
  const container = await buildContainer();
  const { tenantId, branchId } = await getContext(container);
  const app = await buildApp(container);
...
serialTest("Reservation command routes reject invalid lifecycle transitions with 409", async () => {
  const container = await buildContainer();
  const { tenantId, branchId } = await getContext(container);
  const app = await buildApp(container);
  const headers = ownerHeaders(container, tenantId);
  ...
  const createForDoubleConfirm = await app.inject({
    ...
    url: `/v1/branches/${branchId}/reservations`,
    headers,
    payload: { partySize: 2, startAt: "2026-08-11T20:00:00Z", durationMinutes: 60 },
  });
  ...
  const createForSeat = await app.inject({
    ...
    url: `/v1/branches/${branchId}/reservations`,
    headers,
    payload: { partySize: 2, startAt: "2026-08-12T20:00:00Z", durationMinutes: 60 },
  });
  ...
  const createForCancel = await app.inject({
    ...
    url: `/v1/branches/${branchId}/reservations`,
    headers,
    payload: { partySize: 3, startAt: "2026-08-13T20:00:00Z", durationMinutes: 60 },
  });
...
serialTest("Reservation cancel and no-show validate request bodies with 400", async () => {
  const container = await buildContainer();
  const { tenantId, branchId } = await getContext(container);
  const app = await buildApp(container);
  const headers = ownerHeaders(container, tenantId);
  ...
  const create = await app.inject({
    ...
    url: `/v1/branches/${branchId}/reservations`,
    headers,
    payload: { partySize: 2, startAt: "2026-08-14T20:00:00Z", durationMinutes: 60 },
  });
  assert.equal(create.statusCode, 201);
  const reservation = create.json().data;
...
serialTest("Reservation confirm and seat hide cross-tenant reservations as 404", async () => {
  const container = await buildContainer();
  const { tenantId, branchId } = await getContext(container);
  const app = await buildApp(container);
  const now = new Date();
  ...
  const create = await app.inject({
    ...
    url: `/v1/branches/${branchId}/reservations`,
    headers: ownerHeaders(container, tenantId),
    payload: { partySize: 2, startAt: "2026-08-15T20:00:00Z", durationMinutes: 60 },
  });
  assert.equal(create.statusCode, 201);
  const reservation = create.json().data;
...
serialTest("Reservation create validates request body with 400", async () => {
  ...
  const invalidPartySize = await app.inject({
    ...
    url: `/v1/branches/${branchId}/reservations`,
    headers,
    payload: { partySize: 0, startAt: "2026-08-16T20:00:00Z", durationMinutes: 60 },
  });
  ...
  const invalidDuration = await app.inject({
    ...
    url: `/v1/branches/${branchId}/reservations`,
    headers,
    payload: { partySize: 2, startAt: "2026-08-16T20:00:00Z", durationMinutes: 0 },
  });
...
serialTest("Reservation create hides unknown and cross-tenant branches as 404", async () => {
  const container = await buildContainer();
  const { tenantId, branchId } = await getContext(container);
  const app = await buildApp(container);
  const now = new Date();
  ...
  const unknownBranch = await app.inject({
    ...
    url: `/v1/branches/${randomUUID()}/reservations`,
    headers: ownerHeaders(container, tenantId),
    payload: { partySize: 2, startAt: "2026-08-17T20:00:00Z", durationMinutes: 60 },
  });
...
serialTest("Reservation command permissions distinguish confirm cancel no-show and seat", async () => {
  const container = await buildContainer();
  const { tenantId, branchId } = await getContext(container);
  const app = await buildApp(container);
  const now = new Date();
  ...
  const waiterHeaders = { authorization: `Bearer ${waiterToken}`, "x-tenant-id": tenantId };

  const create = await app.inject({
    method: "POST",
    url: `/v1/branches/${branchId}/reservations`,
    headers: ownerHeaders(container, tenantId),
    payload: { partySize: 2, startAt: "2026-08-18T20:00:00Z", durationMinutes: 60 },
  });
  assert.equal(create.statusCode, 201);
  const reservation = create.json().data;
...
serialTest("Reservation confirm returns 409 when branch capacity is unavailable", async () => {
  const container = await buildContainer();
  const { tenantId, branchId } = await getContext(container);
  const app = await buildApp(container);
  const headers = ownerHeaders(container, tenantId);
...
serialTest("Reservation cancel and no-show hide cross-tenant reservations as 404", async () => {
  const container = await buildContainer();
  const { tenantId, branchId } = await getContext(container);
  const app = await buildApp(container);
  const now = new Date();
  ...
  const create = await app.inject({
    ...
    url: `/v1/branches/${branchId}/reservations`,
    headers: ownerHeaders(container, tenantId),
    payload: { partySize: 2, startAt: "2026-08-20T20:00:00Z", durationMinutes: 60 },
  });
  assert.equal(create.statusCode, 201);
  const reservation = create.json().data;
...
serialTest("Reservation command routes return 404 for unknown reservation ids", async () => {
  const container = await buildContainer();
...
serialTest("Reservation read endpoints require reservation:read and allow waiter operational reads", async () => {
  const container = await buildContainer();
  const { tenantId, branchId } = await getContext(container);
  const app = await buildApp(container);
  const now = new Date();
  ...
  const cookHeaders = { authorization: `Bearer ${cookToken}`, "x-tenant-id": tenantId };
...
serialTest("Reservation manager role can confirm cancel no-show and seat", async () => {
  const container = await buildContainer();
  const { tenantId, branchId } = await getContext(container);
  const app = await buildApp(container);
  const now = new Date();
  ...
  const confirmCreate = await app.inject({
    ...
    url: `/v1/branches/${branchId}/reservations`,
    headers: ownerHeaders(container, tenantId),
    payload: { partySize: 2, startAt: "2026-08-22T20:00:00Z", durationMinutes: 60 },
  });
  ...
  const cancelCreate = await app.inject({
    ...
    url: `/v1/branches/${branchId}/reservations`,
    headers: ownerHeaders(container, tenantId),
    payload: { partySize: 3, startAt: "2026-08-23T20:00:00Z", durationMinutes: 60 },
  });
  ...
  const noShowCreate = await app.inject({
    ...
    url: `/v1/branches/${branchId}/reservations`,
    headers: ownerHeaders(container, tenantId),
    payload: { partySize: 4, startAt: "2026-08-24T20:00:00Z", durationMinutes: 60 },
  });
...
serialTest("Waitlist: add, notify, seat, cancel", async () => {
  ...
  const add = await app.inject({
    ...
    headers: ownerHeaders(container, tenantId),
    payload: { partySize: 2, quotedMinutes: 15, notes: "patio", guestId: "guest-waitlist" },
  });
...
serialTest("Waitlist priority override requires dedicated permission", async () => {
  const container = await buildContainer();
  const { tenantId, branchId } = await getContext(container);
  const app = await buildApp(container);
  const now = new Date();
...
serialTest("Waitlist read hides cross-tenant entries as 404 and lists return empty for unknown or cross-tenant branches", async () => {
  const container = await buildContainer();
  const { tenantId, branchId } = await getContext(container);
  const app = await buildApp(container);
  const headers = ownerHeaders(container, tenantId);
...
serialTest("Waitlist list/read are ordered, expire transitions entries, and read permission is distinct from priority override", async () => {
  const container = await buildContainer();
  const { tenantId, branchId } = await getContext(container);
  const app = await buildApp(container);
  const now = new Date();
  ...
  const list = await app.inject({
    method: "GET",
    url: `/v1/branches/${branchId}/waitlist-entries`,
    headers: { authorization: `Bearer ${maitreToken}`, "x-tenant-id": tenantId },
  });
  assert.equal(list.statusCode, 200);
  ...
  assert.equal(priorityOverrideAllowed.json().data.arrivedAt, secondEntry.arrivedAt);
  assert.equal(priorityOverrideAllowed.json().data.priorityOverride, 60);
  assert.equal(priorityOverrideAllowed.json().data.overrideReason, "host desk override");

  await app.close();
});

serialTest("Waitlist command routes hide cross-tenant entries as 404", async () => {
  const container = await buildContainer();
  const { tenantId, branchId } = await getContext(container);
  const app = await buildApp(container);
  const now = new Date();
...
serialTest("Waitlist command routes reject invalid lifecycle transitions with 409", async () => {
  const container = await buildContainer();
  const { tenantId, branchId } = await getContext(container);
  const app = await buildApp(container);
  const headers = ownerHeaders(container, tenantId);
...
serialTest("Waitlist command routes validate seat cancel and priority-override payloads", async () => {
  const container = await buildContainer();
  const { tenantId, branchId } = await getContext(container);
  const app = await buildApp(container);
  const headers = ownerHeaders(container, tenantId);
...
test("Guest: create and anonymize", async () => {
  ...
  assert.equal(anonymize.json().data.updatedAt, anonymize.json().data.anonymizedAt);
...
test("Guest permissions distinguish pii read from anonymize authority", async () => {
  ...
  const create = await app.inject({
    ...
    url: "/v1/guests",
    headers: ownerHeaders(container, tenantId),
    payload: { displayName: "Guest Permissions", email: "guest-perms@example.com" },
  });
  assert.equal(create.statusCode, 201);
  const guest = create.json().data;
...
serialTest("Guest read update and anonymize hide cross-tenant guests as 404", async () => {
  const container = await buildContainer();
  const { tenantId } = await getContext(container);
  const app = await buildApp(container);
  const now = new Date();
  ...
  const create = await app.inject({
    ...
    url: "/v1/guests",
    headers: ownerHeaders(container, tenantId),
    payload: { displayName: "Guest Cross Tenant", email: "guest-cross-tenant@example.com" },
  });
  assert.equal(create.statusCode, 201);
  const guest = create.json().data;
...
serialTest("Availability GET returns free tables", async () => {
  ...
  assert.equal(unavailable.json().data.startAt, new Date("2026-08-01T21:00:00Z").toISOString());
...
serialTest("Availability ignores pending reservations but blocks confirmed reservations and active occupancies", async () => {
  const container = await buildContainer();
  const { tenantId, branchId } = await getContext(container);
  const app = await buildApp(container);
  const headers = ownerHeaders(container, tenantId);
  ...
  const pendingReservation = await app.inject({
    ...
    url: `/v1/branches/${branchId}/reservations`,
    headers,
    payload: { partySize: 2, startAt: "2026-08-05T20:00:00Z", durationMinutes: 60 },
  });
  ...
  const freeTableIds = pendingAvailability.json().data.freeTableIds as string[];
  ...
  const confirmedFreeTableIds = confirmedAvailability.json().data.freeTableIds as string[];
  ...
  const occupancyFreeTableIds = occupancyAvailability.json().data.freeTableIds as string[];
  assert.ok(!occupancyFreeTableIds.includes(occupiedTableId));

  await app.close();
});

serialTest("Availability validates query parameters and returns 404 for unknown branches", async () => {
  const container = await buildContainer();
  const { tenantId } = await getContext(container);
  const app = await buildApp(container);
  const headers = ownerHeaders(container, tenantId);
...
serialTest("Availability requires reservation read permission", async () => {
  ...
  const response = await app.inject({
    method: "GET",
    url: `/v1/branches/${branchId}/availability?partySize=2&startAt=2026-08-05T20:00:00Z&durationMinutes=60`,
    headers: { authorization: `Bearer ${employeeToken}`, "x-tenant-id": tenantId },
  });
...
serialTest("Reservation notifications create and fetch notification intents", async () => {
  const container = await buildContainer();
  const { tenantId, branchId } = await getContext(container);
  const app = await buildApp(container);
  ...
  assert.ok(!Number.isNaN(Date.parse(cancellationIntent.json().data.createdAt)));
...
serialTest("Reservation notifications require notification permission and reservation existence", async () => {
  const container = await buildContainer();
  const { tenantId } = await getContext(container);
  const app = await buildApp(container);
  const now = new Date();
...
serialTest("Notification intent detail requires notification permission and hides cross-tenant intents as 404", async () => {
  const container = await buildContainer();
  const { tenantId, branchId } = await getContext(container);
  const app = await buildApp(container);
  const now = new Date();
  ...
  const createReservation = await app.inject({
    ...
    url: `/v1/branches/${branchId}/reservations`,
    headers: ownerHeaders(container, tenantId),
    payload: { partySize: 2, startAt: "2026-08-07T20:00:00Z", durationMinutes: 60 },
  });
  assert.equal(createReservation.statusCode, 201);
  const reservation = createReservation.json().data;
...
serialTest("Notification intent creation hides cross-tenant reservations as 404 for every intent endpoint", async () => {
  const container = await buildContainer();
  const { tenantId, branchId } = await getContext(container);
  const app = await buildApp(container);
  const headers = ownerHeaders(container, tenantId);
  ...
  const createReservation = await app.inject({
    ...
    url: `/v1/branches/${branchId}/reservations`,
    headers,
    payload: { partySize: 2, startAt: "2026-08-08T20:00:00Z", durationMinutes: 60 },
  });
  assert.equal(createReservation.statusCode, 201);
  const reservation = createReservation.json().data;
...
serialTest("Notification intent detail returns 404 for unknown ids", async () => {
  const container = await buildContainer();
  const { tenantId } = await getContext(container);
  const app = await buildApp(container);
...
serialTest("Reservation preferences: create and list guest/reservation scoped preferences", async () => {
  const container = await buildContainer();
  const { tenantId, branchId } = await getContext(container);
  const app = await buildApp(container);
  const headers = ownerHeaders(container, tenantId);
  ...
  assert.ok(!Number.isNaN(Date.parse(reservationRequirement.json().data.createdAt)));
  assert.equal(typeof reservationRequirement.json().data.updatedAt, "string");
  assert.ok(!Number.isNaN(Date.parse(reservationRequirement.json().data.updatedAt)));
  ...
  assert.equal(typeof guestRequirementWithoutValue.json().data.createdAt, "string");
  assert.ok(!Number.isNaN(Date.parse(guestRequirementWithoutValue.json().data.createdAt)));
  assert.equal(typeof guestRequirementWithoutValue.json().data.updatedAt, "string");
  assert.ok(!Number.isNaN(Date.parse(guestRequirementWithoutValue.json().data.updatedAt)));
  assert.equal(guestRequirementWithoutValue.json().data.revision, 1);
  assert.deepEqual(Object.keys(guestRequirementWithoutValue.json().data).sort(), [
    "code",
    "createdAt",
    "id",
    "kind",
  ...
  assert.equal(guestList.json().data[0].createdAt, guestPreference.json().data.createdAt);
  assert.equal(guestList.json().data[0].updatedAt, guestPreference.json().data.updatedAt);
  ...
  assert.equal(secondGuestList.json().data[0].id, guestRequirementWithoutValue.json().data.id);
  assert.equal(secondGuestList.json().data[0].createdAt, guestRequirementWithoutValue.json().data.createdAt);
  assert.equal(secondGuestList.json().data[0].updatedAt, guestRequirementWithoutValue.json().data.updatedAt);
  ...
  assert.equal(reservationList.json().data[0].id, reservationRequirement.json().data.id);
  assert.equal(reservationList.json().data[0].createdAt, reservationRequirement.json().data.createdAt);
  assert.equal(reservationList.json().data[0].updatedAt, reservationRequirement.json().data.updatedAt);
...
serialTest("Reservation preferences enforce create/read permissions and query validation", async () => {
  const container = await buildContainer();
  const { tenantId } = await getContext(container);
  const app = await buildApp(container);
  const now = new Date();
...
serialTest("Reservation preferences list returns empty across tenants for the same subject", async () => {
  const container = await buildContainer();
  const { tenantId } = await getContext(container);
  const app = await buildApp(container);
  const now = new Date();
...
serialTest("Cancellation policy: upsert, get current and evaluate", async () => {
  const container = await buildContainer();
  const { tenantId } = await getContext(container);
  const app = await buildApp(container);
  const headers = ownerHeaders(container, tenantId);
  ...
  assert.equal(evaluateWithoutPolicy.json().data.withinFreeCancellationWindow, true);
  ...
  assert.equal(updatedCurrent.json().data.createdAt, create.json().data.createdAt);
  assert.equal(updatedCurrent.json().data.updatedAt, update.json().data.updatedAt);
...
serialTest("Cancellation policy enforces create/read permissions and validates body/query", async () => {
  const container = await buildContainer();
  const { tenantId } = await getContext(container);
  const app = await buildApp(container);
  const now = new Date();
...
serialTest("Cancellation policy current and evaluate are isolated by tenant", async () => {
  const container = await buildContainer();
  const { tenantId } = await getContext(container);
  const app = await buildApp(container);
  const now = new Date();

...
| Operación | Resultado |
| --- | --- |
| `POST /v1/branches` | crea sucursal `ACTIVE`; requiere brand y fiscal entity accesibles |
| `GET /v1/branches` | lista estable paginada, filtrable por brand/status |
| `GET /v1/branches/{branchId}` | devuelve sucursal sin expandir agregados por defecto |
| `PATCH /v1/branches/{branchId}` | actualiza campos mutables con control de concurrencia |

No existe eliminación física en este contrato. Desactivación usa `status: INACTIVE` y falla si
...
- filtros no revelan existencia de otro tenant;
...
fuera de alcance; `409` unicidad/cuota/idempotencia; `412` versión; `422` regla de dominio.
...
- Negativos de otro tenant indistinguibles de recurso inexistente.

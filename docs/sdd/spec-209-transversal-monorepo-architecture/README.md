# [SPEC-209] Monorepo Architecture

Contrato estructural para implementar Maitre con React.js, Node.js y TypeScript sin acoplar el núcleo a Vercel ni a proveedores aún no seleccionados.

| Campo | Valor |
| --- | --- |
| **ID** | SPEC-209 |
| **Tipo** | Transversal / Architecture |
| **Dominio** | Platform / Engineering |
| **Estado** | DRAFT |
| **Readiness** | READY_FOR_I0_REVIEW |
| **Prioridad** | P0 |
| **Owner** | UNASSIGNED |
| **Reviewer** | UNASSIGNED |
| **Fase** | Antes del scaffolding |
| **Depende de** | SPEC-207, SPEC-208 |

## Documentos

- [Objetivo](objective.md)
- [Estructura y contratos](specification.md)
- [Reglas de dependencias](rules.md)
- [Plan](plan.md)
- [Tareas](tasks.md)
- [Verificación](verification.md)
- [Decisiones abiertas](notes.md)
- [Matriz de dependencias](dependency-boundaries.md)

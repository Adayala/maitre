...
Roles y alcances por sucursal son assignments normalizados/versionados; no arrays confiados al
cliente ni datos almacenados en User.

## Campos y lifecycle

Campos: id, tenantId, userId, status `INVITED | ACTIVE | SUSPENDED | REVOKED`, role
assignments, alcances por sucursal, invitation reference, version y auditoría.
...
2. Roles/permissions pertenecen al catálogo y los alcances a Branches del mismo tenant.
3. Debe quedar al menos un OWNER activo; transferencia usa command concurrente/auditado.
4. Actor no delega capacidades/alcances superiores a los propios.
...
Tests cubren invitación concurrente, aceptación, last owner, self-grant, alcance por sucursal,
